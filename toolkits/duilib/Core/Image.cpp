...
ImageInfo::ImageInfo()
    : m_bAlphaChannel(false), m_bCached(false), m_propertyItem(), m_vecBitmap() {}
...
void ImageInfo::PushBackHBitmap(HBITMAP hBitmap) { m_vecBitmap.push_back(hBitmap); }

HBITMAP ImageInfo::GetHBitmap(int nIndex) { return m_vecBitmap[nIndex]; }

int ImageInfo::GetFrameCount() { return (int) m_vecBitmap.size(); }

bool ImageInfo::IsGif() { return m_vecBitmap.size() > 1; }

int ImageInfo::GetInterval(int nIndex) {
    if (m_propertyItem == nullptr) return 0;

    if (nIndex >= (int) m_vecBitmap.size()) return 0;
...
std::unique_ptr<ImageInfo> ImageInfo::LoadImage(HGLOBAL hGlobal,
...
ImageInfo::LoadImageByBitmap(std::unique_ptr<Gdiplus::Bitmap> &pGdiplusBitmap,
...
ImageAttribute::ImageAttribute() { Init(); }
    ...
    while (*pStrImage != _T('\0')) {
        ...
        if (!sValue.empty()) {
            if (sItem == _T("file") || sItem == _T("res")) {
                UINT scaleOfDpi = DpiManager::GetInstance()->GetScale();
                if (scaleOfDpi > 100) {
                    size_t pos = sValue.rfind(L"/");
                    std::wstring scaleMark(L"X" + std::to_wstring(scaleOfDpi));
                    sValue.insert(pos, scaleMark);
                }
                ...
                if (bScaleDest) DpiManager::GetInstance()->ScaleRect(imageAttribute.rcDest);
...
Image::Image()
    : imageAttribute(), imageCache(), m_nCurrentFrame(0), m_bPlaying(false), m_nCycledCount(0) {}
...
void Image::SetCurrentFrame(int nCurrentFrame) { m_nCurrentFrame = nCurrentFrame; }
...
int Image::GetCurrentFrameIndex() { return m_nCurrentFrame; }
int Image::GetCycledCount() { return m_nCycledCount; }
void Image::ClearCycledCount() { m_nCycledCount = 0; }
...
StateImage::StateImage() : m_pControl(nullptr), m_stateImageMap() {}

bool StateImage::HasHotImage() {
    return !m_stateImageMap[kControlStateHot].imageAttribute.simageString.empty();
}

bool StateImage::PaintStatusImage(IRenderContext *pRender, ControlStateType stateType,
                                  const std::wstring &sImageModify /*= L""*/) {
    if (m_pControl) {
        bool bFadeHot =
            m_pControl->GetAnimationManager().GetAnimationPlayer(kAnimationHot) != nullptr;
        int nHotAlpha = m_pControl->GetHotAlpha();
        if (bFadeHot) {
            if (stateType == kControlStateNormal || stateType == kControlStateHot) {
                std::wstring strNormalImagePath =
                    m_stateImageMap[kControlStateNormal].imageAttribute.sImageName;
                ...
                if (strNormalImagePath.empty() || strHotImagePath.empty() ||
                    ...
                    !m_stateImageMap[kControlStateNormal].imageAttribute.rcSource.Equal(
                        m_stateImageMap[kControlStateHot].imageAttribute.rcSource)) {

                    m_pControl->DrawImage(pRender, m_stateImageMap[kControlStateNormal],
                                          sImageModify);
                    int nHotFade = m_stateImageMap[kControlStateHot].imageAttribute.bFade;
                    nHotFade = int(nHotFade * (double) nHotAlpha / 255);
                    return m_pControl->DrawImage(pRender, m_stateImageMap[kControlStateHot],
                                                 sImageModify, nHotFade);
                } else {
                    int nNormalFade = m_stateImageMap[kControlStateNormal].imageAttribute.bFade;
                    int nHotFade = m_stateImageMap[kControlStateHot].imageAttribute.bFade;
                    int nBlendFade = int((1 - (double) nHotAlpha / 255) * nNormalFade +
...
void StateImageMap::SetImage(StateImageType stateImageType, ControlStateType stateType,
...
bool StateImageMap::PaintStatusImage(IRenderContext *pRender, StateImageType stateImageType,
                                     ControlStateType stateType,
                                     const std::wstring &sImageModify /*= L""*/) {
...
StateColorMap::StateColorMap() : m_pControl(nullptr), m_stateColorMap() {}

void StateColorMap::SetControl(Control *control) { m_pControl = control; }

bool StateColorMap::HasHotColor() { return !m_stateColorMap[kControlStateHot].empty(); }

void StateColorMap::PaintStatusColor(IRenderContext *pRender, UiRect rcPaint,
                                     ControlStateType stateType) {
    if (m_pControl) {
        bool bFadeHot =
            m_pControl->GetAnimationManager().GetAnimationPlayer(kAnimationHot) != nullptr;
